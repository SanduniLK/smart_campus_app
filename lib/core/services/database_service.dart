...
class DatabaseService {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    // Events Table - FIXED: Added CREATE TABLE keyword
    await db.execute('''
      CREATE TABLE events(
  ...
  Future<void> insertOrUpdateTimetableEntry(TimetableEntry entry) async {
    final db = await database;
    await db.insert('timetable', entry.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<List<TimetableEntry>> getUnsyncedTimetableEntries() async {
    final db = await database;
    final result = await db.query('timetable', where: 'isSynced = 0');
  ...
  Future<void> updateTimetableFirestoreId(int localId, String firestoreId) async {
    final db = await database;
    await db.update('timetable', {'firestoreId': firestoreId}, where: 'id = ?', whereArgs: [localId]);
  }

  Future<void> markTimetableAsSynced(int localId) async {
    final db = await database;
    await db.update('timetable', {'isSynced': 1}, where: 'id = ?', whereArgs: [localId]);
  }
  ...
  Future<List<Map<String, dynamic>>> getAllEvents() async {
    final db = await database;
    return await db.query('events', distinct: true, orderBy: 'eventDate DESC');
  }
  ...
  Future<void> updateEventFirestoreId(int localId, String firestoreId) async {
    final db = await database;
    await db.update('events', {'firestoreId': firestoreId}, where: 'id = ?', whereArgs: [localId]);
  }

  Future<void> updateEventSyncStatus(int localId, bool isSynced) async {
    final db = await database;
    await db.update('events', {'isSynced': isSynced ? 1 : 0}, where: 'id = ?', whereArgs: [localId]);
  }
  ...
  Future<void> insertOrUpdateEvent(Map<String, dynamic> eventData) async {
    ...
    if (existingEvent.isNotEmpty) {
      await db.update('events', eventData, where: 'id = ?', whereArgs: [existingEvent.first['id']]);
  ...
  Future<String> registerForEvent(int eventId, String userId) async {
    ...
    await db.rawUpdate('UPDATE events SET registeredCount = registeredCount + 1 WHERE id = ?', [eventId]);
  ...
  Future<Map<String, dynamic>?> getEventByFirestoreId(String firestoreId) async {
    final db = await database;
    final result = await db.query('events', where: 'firestoreId = ?', whereArgs: [firestoreId]);
    return result.isNotEmpty ? result.first : null;
  }

  Future<List<Map<String, dynamic>>> getUnsyncedEvents() async {
    final db = await database;
    return await db.query('events', where: 'isSynced = 0 OR isSynced IS NULL');
  }

  // ==================== CAMPUS BUILDINGS OPERATIONS ====================

  Future<void> insertCampusBuilding(Map<String, dynamic> building) async {
    final db = await database;
    await db.insert('campus_buildings', building, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<Map<String, dynamic>?> getCampusBuildingById(String id) async {
    final db = await database;
    final result = await db.query('campus_buildings', where: 'id = ?', whereArgs: [id]);
    return result.isNotEmpty ? result.first : null;
  }
  ...
  Future<List<Course>> getUnsyncedCourses() async {
    final db = await database;
    final result = await db.query('courses', where: 'isSynced = 0');
